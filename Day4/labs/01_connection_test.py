...
try:
    ...
    repsonse2 = client2.chat.completions.create(
        model = "gpt-4o-mini",
        messages=[
            {"role" : "user", "content": "Explain the difference between a legacy SDK and a modern SDK."}
        ]
    )
    
    print("\n--- OPENAI GPT 4o mini Response ---")
    print(repsonse2.choices[0].message.content)
